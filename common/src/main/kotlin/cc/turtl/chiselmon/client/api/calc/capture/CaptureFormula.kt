...
fun estimateCaptureProbability(
    ...
    val ctx =
        CaptureContext(
            ...
            playerActiveBattlePokemon =
                battle
                    ?.getParticipatingActor(player.uuid)
                    ?.activePokemon
                    ?.mapNotNull { it.battlePokemon } ?: emptyList(),
            targetStatus =
                battle
                    ?.wildActor
                    ?.activePokemon
    ...
    val difficulty =
        if (maxPartyLevel == null || maxPartyLevel >= pokemon.level) {
            1f
        } else {
...
fun calculateCapture(
    params: CaptureParams,
    difficultyModifier: Float,
): Float {
    val rate =
        (
            (3f * params.maxHp - 2f * params.clampedHp) / (3f * params.maxHp) *
                params.darkGrassModifier * params.catchRate * params.inBattleModifier *
                params.ballBonus * params.statusMultiplier * params.levelBonus
        ) * difficultyModifier
...
private fun statusMultiplier(status: PersistentStatus?): Float =
    when (status) {
...
private fun levelBonus(level: Int): Float = if (level < LOW_LEVEL_THRESHOLD) ((36f - 2f * level) / 10f).coerceAtLeast(1f) else 1f
