...
public final class PokemonPredicates {
    ...
    public static final Predicate<Pokemon> IS_SHOULDERABLE = p -> {
        ClientSpecies clientSpecies = ClientSpeciesRegistry.get(p.getSpecies().getName());
                if (clientSpecies == null) return false;

                return clientSpecies.shoulderMountable();
    };
