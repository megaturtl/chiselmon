...
public class H2Adapter<T> implements StorageAdapter<T> {
    ...
    private final BiFunction<Connection, Path, T> factory;
    private final Consumer<T> onSave;
    private final Consumer<T> onClose;

    private H2Adapter(String filename, BiFunction<Connection, Path, T> factory, Consumer<T> onSave, Consumer<T> onClose) {
    ...
    public static <T> H2Adapter<T> of(String filename, BiFunction<Connection, Path, T> factory, Consumer<T> onSave, Consumer<T> onClose) {
     ...
     */
    public static <T> H2Adapter<T> of(String filename, BiFunction<Connection, Path, T> factory, Consumer<T> onClose) {
        return new H2Adapter<>(filename, factory, t -> {}, onClose);
    }
    ...
    public T load(StorageScope scope) {
        try {
            ...
            return factory.apply(ds.getConnection(), file);
        } catch (Exception e) {
