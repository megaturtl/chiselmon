...
- **Access**: Provide a single static accessor `CobbleAid.services()` returning the context; other classes request specific services (`CobbleAid.services().config()`, `CobbleAid.services().worldData()`, `CobbleAid.services().logging()`, with a short alias like `CobbleAid.services().logger(Class<?>)`). Avoid caching config snapshots; prefer lightweight accessors or listeners.
...
3. **Migrate consumers incrementally**
   ...
   - Replace `CobbleAid.getLogger()` usages with `CobbleAid.services().logging().getLogger(Class<?>)` (or the alias `CobbleAid.services().logger(Class<?>)` for brevity).
